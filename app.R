...
#'
#' Determines if a column can be treated as numeric, even if stored as character.
#' Treats empty/NA-only columns as numeric-like so new trick headers can be added.
#'
#' @param x A vector or column to check.
#' @return TRUE if the column is numeric or can be coerced to numeric, FALSE otherwise.
is_numeric_like <- function(x) {
  if (is.list(x)) return(FALSE)
  if (is.numeric(x)) return(TRUE)
  # Treat empty/NA-only columns as numeric-like so new trick headers can be added
  vals <- as.character(x)
...
server <- function(input, output, session) {
  ...
  logs_trigger <- reactiveVal(Sys.time())  # manual refresh trigger

  logs <- reactive({
    autoInvalidate()          # invalidates this reactive every minute
    logs_trigger()            # ensure a manual refresh trigger exists
  ...
  observe({
    ...
    updateSelectInput(session, "trick",
      choices = tr,
      selected = input$trick %||% if (length(tr) > 0) tr[1] else NULL
    )
  ...
  observeEvent(input$add, {
    ...
    df <- read_logs()
    date_col <- "date"  # read_logs() always ensures "date" column exists
